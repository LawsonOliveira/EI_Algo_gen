...
class Node:  # Generic tree node
    ...
    def __init__(self, table=__ORIGINAL_ROT_TABLE, interval=__ORIGINAL_INTERVALS):
        ...
        self.__Table = {}
        for dinucleotide in self.__Rot_Table:
            #iniciate the values randomly
            self.__Table[dinucleotide] = [np.random.uniform(self.__Rot_Table[dinucleotide][0]-self.__Rot_Table[dinucleotide][3],self.__Rot_Table[dinucleotide][0]+self.__Rot_Table[dinucleotide][3]),np.random.uniform(self.__Rot_Table[dinucleotide][1]-self.__Rot_Table[dinucleotide][4],self.__Rot_Table[dinucleotide][1]+self.__Rot_Table[dinucleotide][4]),np.random.uniform(self.__Rot_Table[dinucleotide][2]-self.__Rot_Table[dinucleotide][5],self.__Rot_Table[dinucleotide][2]+self.__Rot_Table[dinucleotide][5])]
    ...
    def getTwist(self, dinucleotide):
        return self.__Table[dinucleotide][0]

    def getWedge(self, dinucleotide):
        return self.__Table[dinucleotide][1]

    def getDirection(self, dinucleotide):
        return self.__Table[dinucleotide][2]
